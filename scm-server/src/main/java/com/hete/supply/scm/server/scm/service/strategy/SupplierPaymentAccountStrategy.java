...
public class SupplierPaymentAccountStrategy implements WorkflowMqBackStrategy {
    ...
    public void workflowMqBackHandleBusiness(WorkflowMqCallbackDto dto, FeishuAuditOrderPo feishuAuditOrderPo) {
        ...
        if (supplierPaymentAccountPo != null) {
            if (AGREE.equals(dto.getWorkflowResult())) {
                supplierPaymentAccountPo.setSupplierPaymentAccountStatus(SupplierPaymentAccountStatus.EFFECTIVE);
                supplierPaymentAccountDao.updateByIdVersion(supplierPaymentAccountPo);
                // 日志
                supplierPaymentAccountBaseService.createStatusChangeLog(supplierPaymentAccountPo, "生效审批通过", null);
            }
            if (REFUSE.equals(dto.getWorkflowResult())) {
                ...
                supplierPaymentAccountDao.updateByIdVersion(supplierPaymentAccountPo);
                // 日志
                supplierPaymentAccountBaseService.createStatusChangeLog(supplierPaymentAccountPo, "生效审批不通过", null);
            }
            // 撤销、失败状态处理
    ...
    public FeishuWorkflowType getHandlerType() {
        return FeishuWorkflowType.SUPPLIER_PAYMENT_ACCOUNT;
    }
}
